...
func TestListFilter(t *testing.T) {
	...
	list, total, err := s.QueryPage(ListQuery{Filter: `(and (= type "article") (= status 1))`, Page: 1, Size: 10})
	if err != nil {
		t.Fatal(err)
	}
	...
	// 分页: draft 1 条
	list, total, _ = s.QueryPage(ListQuery{Filter: `(and (= type "article") (= status 0))`, Page: 1, Size: 10})
	if total != 1 {
		t.Fatalf("draft total: %d", total)
	}
	// 不过滤
	_, total, _ = s.QueryPage(ListQuery{Filter: `(= type "article")`, Page: 1, Size: 10})
	if total != 2 {
		t.Fatalf("all total: %d", total)
	}
	// 按类型隔离
	_, total, _ = s.QueryPage(ListQuery{Filter: `(= type "person")`, Page: 1, Size: 10})
	if total != 1 {
